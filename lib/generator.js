...
module.exports = function(locals) {
    ...
    Anything.prototype.getIndexPage = function() {
      ...
      let ret = _.map(_.keys(that.posts), function(key) {
        ...
        // try to get markdown file for each key from /source/_anything/<index>/<key>.md
        ...
        if (fs.existsSync(mdSource)) {
          ...
          data = Object.assign(data, front.parse(md));
    ...
    Anything.prototype.getPages = function() {
      ...
      return _.map(that.posts, function(posts, key) {

        let ret = {
          data: {
            index: {
              name: that.index,
              caption: S(that.index).humanize().titleCase().s
            },
            key: {
              name: key,
        ...
        if (key !== 'index') { 
          let keyTitle = _.find(that.posts.index, function(i) { return i.key === key; }).title;
          ret.data.key.caption = keyTitle;
          ret.data.title = ret.data.index.caption + config.anything.titleSeparator + ret.data.key.caption;
          ret.layout = config.anything.layout_posts || 'index'
        } else {
          ret.data.title = ret.data.index.caption;
          ret.layout = config.anything.layout_index || 'index'
        }
