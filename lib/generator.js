...
module.exports = function(locals) {
    ...
    Anything.prototype.getPath = function(key) {
      ...
      let root = utils.slugize(this.path);
    ...
    Anything.prototype.getMarkdownData = function(key) {
      ...
      const mdSource = path.join(config.source_dir, "_anything", that.index.toLowerCase(), key.toLowerCase() + ".md");
    ...
    Anything.prototype.getPages = function() {
      ...
      let ret = _.map(that.posts, function(posts, key) {
        ...
        if (key === 'index') {
          ...
          if (!page.data.title) page.data.title = _.startCase(human(that.index));
